...
void macro_GenerateLightEfficiencyCurves4_VEDA()
{
  int Z=1;
  int A=1;
...
//  std::string FileOutTag("InVoltage");
  ...
  ifstream FileIn(Form("data_VEDA/DEEFITPointsFullInfo_VEDA.dat"));
  ofstream FileOut(Form("data_VEDA/VEDALightEfficiencyData%s_Z%02d_A%02d.dat",FileOutTag.c_str(), Z, A));
  ...
  for(int i=0; i<12; i++) {
    for(int j=0; j<4; j++) {
      ...
      LightEnergyGraph[i][j]= new TGraph(EnergyValues[i][j].size(),EnergyValues[i][j].data(),LightOutput[i][j].data());
