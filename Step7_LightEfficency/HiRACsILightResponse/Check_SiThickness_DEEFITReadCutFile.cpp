...
void Check_SiThickness_DEEFITReadCutFile()
{
  ...
  ofstream FileOut("Check_SiThickness/data/DEEFITPointsTel05_00_1600um.dat");
  ...
  TelSiThickness[4]=1517; /*TelSiThickness[5]=1512;*/ TelSiThickness[5]=1600;
  TelSiThickness[6] =1491;TelSiThickness [7]=1460;
 ...
 //----- Loading cut ranges to take DEE points ------------------
  while (FileInCut.is_open() && !FileInCut.eof())
  {
    ...
    for(int XChannel=CutStart; XChannel<CutEnd; XChannel+=CutStep)
    {
      ...
      printf("Tel %02d CsI %02d  Z%02d  A%02d : Vol=%.3f ESi=%.3f +- %.4f -> ECsI=%.3f +- %.4f\n",numtel, numcsi, Z, A,CsIV, ESi, errESi, CsIE, errCsIE);
    }
  }
  FileOut.close();
}
