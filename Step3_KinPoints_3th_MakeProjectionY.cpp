...
void Step3_KinPoints_3th_MakeProjectionY()
{

   TCanvas *c1=new TCanvas("c1","",1200,800);
   c1->Divide(2,1);

   TFile *FileIn=new TFile(Form("RootData/NewGeo_HiRA_KinematicPlots_%04d_%04d_%02dCa+1H@%.1fAMeV.root",FirstRun, LastRun, A_beam, E_projectile));
   TFile *FileInCut=new TFile(Form("RootData/NewGeo_HiRA_KinematicPlotsCut_%04d_%04d_%02dCa+1H@%.1fAMeV.root",FirstRun, LastRun, A_beam, E_projectile));

   ofstream FileOut(Form("Step3_KinematicPoints/Cuts&Projection/NewGeo_HiRA_CsIKinematics_%4d_%4d_%dCa%.1fAMeV_gain%4d.dat", FirstRun, LastRun, A_beam, E_projectile, csi_gain_setting));
   ofstream FileOutFull(Form("Step3_KinematicPoints/Cuts&Projection/NewGeo_HiRA_CsIKinematics_FullInfo_%4d_%4d_%dCa%.1fAMeV_gain%4d.dat", FirstRun, LastRun, A_beam, E_projectile, csi_gain_setting));
...
double ThetaMean[100];
  ...
  for(int TelNum=0; TelNum<12; TelNum++)
  {
    for(int CsINum=0; CsINum<4; CsINum++)
    {
      ...
      gPad->Modified();
      gPad->Update();
      c2->Print(Form("12KinematicLineProjection/DataPoints_%04d_%04d/ProjCsI_%02d_%02d_%04d_%04d.png",FirstRun, LastRun,TelNum, CsINum, FirstRun, LastRun));
