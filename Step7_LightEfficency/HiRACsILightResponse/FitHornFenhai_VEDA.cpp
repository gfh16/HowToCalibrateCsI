...
////////////////////////////////////////////////////////////////////////////////
/// definition of Horn formula saturation corrected by Fenhai
/// this formula is given in the following paper:
//// D.Horn et al, NIM A320(1992) 273-276
//// and has been modified by Fenhai to account for high-energy saturation
//// visible in light particles Z<=3
////////////////////////////////////////////////////////////////////////////////
double HornFenhaiFit(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  if(x[1]>200)
  {
    int A = Int_t(x[1])%100;
    int Z = Int_t(x[1])/100;
    double exponent = (A*pow(Z,2)+par[2])/(A*pow(Z,2)+par[3]);
    double consterm = par[4]*A*pow(Z,2);
    double lineterm = x[0]+consterm;
    double logterm = consterm*log(abs(lineterm/consterm));
    double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
    return light;
  }
...
double fit_HornFenhai_He3(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (3*pow(2,2)+par[2])/(3*pow(2,2)+par[3]);
  double consterm = par[4]*3*pow(2,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_He4(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (4*pow(2,2)+par[2])/(4*pow(2,2)+par[3]);
  double consterm = par[4]*4*pow(2,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_He6(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (6*pow(2,2)+par[2])/(6*pow(2,2)+par[3]);
  double consterm = par[4]*6*pow(2,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_Li6(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (6*pow(3,2)+par[2])/(6*pow(3,2)+par[3]);
  double consterm = par[4]*6*pow(3,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_Li7(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (7*pow(3,2)+par[2])/(7*pow(3,2)+par[3]);
  double consterm = par[4]*7*pow(3,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_Li8(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (8*pow(3,2)+par[2])/(8*pow(3,2)+par[3]);
  double consterm = par[4]*8*pow(3,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_Be7(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (7*pow(4,2)+par[2])/(7*pow(4,2)+par[3]);
  double consterm = par[4]*7*pow(4,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
double fit_HornFenhai_Be9(double *x, double *par)
{
  if(par[2]>=par[3])
  {
    return -1;
  ...
  double exponent = (9*pow(4,2)+par[2])/(9*pow(4,2)+par[3]);
  double consterm = par[4]*9*pow(4,2);
  double lineterm = x[0]+consterm;
  double logterm = consterm*log(abs(lineterm/consterm));
  double light = par[0]+par[1]*(pow(x[0],exponent)-logterm);
  return light;
...
//------------------------------------------------------------------------------
void FitHornFenhai_VEDA()
{
  ofstream FileOut;
  FileOut.open("calibrations/VEDA_SimultaneouslyFitParHeavyIons.dat");
  ...
  FileIn_name_He3[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z02_A03.dat"); //DEE points
  ...
  FileIn_name_He4[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z02_A04.dat"); //DEE points
  FileIn_name_He4[1] = new std::string("data_VEDA/WMUdataAddOnePecenterrEnergy_Z02_A04.dat");
  ...
  FileIn_name_He6[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z02_A06.dat"); //DEE points
  ...
  FileIn_name_Li6[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z03_A06.dat"); //DEE points
  ...
  FileIn_name_Li7[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z03_A07.dat"); //DEE points
  ...
  FileIn_name_Li8[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z03_A08.dat"); //DEE points
  ...
  FileIn_name_Be7[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z04_A07.dat"); //DEE points
  ...
  FileIn_name_Be9[0] = new std::string("data_VEDA/VEDA_DEEPointsInVoltage_Z04_A09.dat"); //DEE points
...
//______________________________________________________________________________
  ...
  TF2 *fHeavyIon= new TF2("fHeavyIon",HornFenhaiFit,0,500,200,500,5);
  fHeavyIon->SetParameters(0.001,0.01,10000,12000,0.1);

  TF1 * fHe3 = new TF1("fHe3",fit_HornFenhai_He3,0,500,5);
  TF1 * fHe4 = new TF1("fHe4",fit_HornFenhai_He4,0,500,5);
  TF1 * fHe6 = new TF1("fHe6",fit_HornFenhai_He6,0,500,5);
  TF1 * fLi6 = new TF1("fLi6",fit_HornFenhai_Li6,0,500,5);
  TF1 * fLi7 = new TF1("fLi7",fit_HornFenhai_Li7,0,500,5);
  TF1 * fLi8 = new TF1("fLi8",fit_HornFenhai_Li8,0,500,5);
  TF1 * fBe7 = new TF1("fBe7",fit_HornFenhai_Be7,0,500,5);
  TF1 * fBe9 = new TF1("fBe9",fit_HornFenhai_Be9,0,500,5);
...
//______________________________________________________________________________
  ...
  for(int i=0; i<12; i++)
  {
    for(int j=0; j<4; j++)
    {
      ...
      double par3 = fHeavyIon->GetParameter(3);
      double par4 = fHeavyIon->GetParameter(4);
      int Z;
      int A;
      for(int k=0; k<ZA_HeavyIons[i][j].size();k++)
      {
        ...
        FileOut << setw(5) << i <<"  "<< setw(5) << j <<"  "<<setw(5)<< Z <<"  "<<setw(5)<< A <<"  "<< setw(35)<< Form("[0]+[1].(x^((%d.pow(%d,2)+[2])/(%d.pow(%d,2)+[3]))-[4].%d.pow(%d,2).log((x+[4].%d.pow(%d,2))/([4].%d.pow(%d,2))))", A, Z, A, Z, A, Z, A, Z, A, Z) <<"  "<< setw(10) << 5 << "  " << setw(15) << par0 <<"  "<< setw(10) << par1 <<"  "<<setw(10)<< par2 << "  "<< setw(10) << par3 <<"  "<<setw(10)<< par4 <<endl;
      }
    }
  }
//______________________________________________________________________________
