...
{
  NucData=new nuclear_masses("LISETools/masses.conf");
}
...
double EnergyLossModule::GetEnergyLoss(int Z, int A, double Einc, const char * material, double thickness_um, int model)
{
  ...
  if(LoadEnergyLossFile(Form("LISETools/input/LISE_ELoss_Z%02d_A%02d_%s.dat", Z, A, material))<=0) {
...
double EnergyLossModule::GetRangeFromEnergy(int Z, int A, double Einc, const char * material, int model)
{
  ...
  if(LoadRangeFile(Form("LISETools/input/LISE_Range_Z%02d_A%02d_%s.dat", Z, A, material))<=0) {
    printf("Error: information not present for Z=%d A=%d material=%s\n", Z, A, material);
    return -100;
  }
...
{
  if(LoadEnergyLossFile(Form("LISETools/input/LISE_ELoss_Z%02d_A%02d_%s.dat", Z, A, material))<=0) return;
